...
from old.models_2 import ImprovedSRCNN, Discriminator, ContentLoss
...
def train_gan_model(generator, discriminator, train_loader, val_loader, test_set5_loader, test_set14_loader, num_epochs,
    ...
    optimizer_G = optim.AdamW(generator.parameters(), lr=0.0001, betas=(0.9, 0.999), weight_decay=0.0001)
    optimizer_D = optim.AdamW(discriminator.parameters(), lr=0.00005, betas=(0.9, 0.999), weight_decay=0.0001)
    
    steps_per_epoch = len(train_loader)
    total_steps = num_epochs * steps_per_epoch

    scheduler_G = torch.optim.lr_scheduler.OneCycleLR(
        optimizer_G, max_lr=0.0001, total_steps=total_steps, pct_start=0.2, anneal_strategy='cos', cycle_momentum=False
    )
    scheduler_D = torch.optim.lr_scheduler.OneCycleLR(
        optimizer_D, max_lr=0.00005, total_steps=total_steps, pct_start=0.2, anneal_strategy='cos',cycle_momentum=False
    )
    ...
    for epoch in range(num_epochs):
	    ...
	    #train generator first	
            ...
            # Reduced adversarial impact
            g_loss = content_loss + 0.00001 * adversarial_loss  # Reduced from 0.0001
            ...
            if batch_size % 2 == 0:  # Train D every other batch
...
def plot_training_curves(history):
    ...
    plt.tight_layout()
    plt.savefig('training_curves_2.png')
    plt.close()


def save_training_history(history, filename='training_history_2.txt'):
...
if __name__ == "__main__":
    ...
    with open('old/final_results_x4_2.txt', 'w') as f:
